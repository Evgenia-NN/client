...
<template>
    <div class="relative isolate overflow-hidden dark:bg-neutral-700">
        <h1 class="text-rose-500 text-center text-2xl sm:text-3xl md:text-4xl font-bold p-4">Портфолио</h1>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mx-4 sm:mx-8 md:mx-12 lg:mx-16 xl:mx-40 py-4 sm:py-6 md:py-8">
            <article v-for="work in works" :key="work.id" class="flex flex-col p-4 sm:p-5 max-w-sm bg-rose-50/50 border border-rose-500 rounded-lg shadow-sm">
                ...
                <div class="inline-flex flex-col gap-2 sm:gap-3 p-2">
                    ...
                    <NuxtLink :to="`/portfolio/${work.slug}`" class="flex items-center text-sm sm:text-base">
                        <p>Подробнее</p>
                    ...
                    <div class="mt-2 sm:mt-4 flex items-center justify-between">
                        <span class="text-xs sm:text-sm text-gray-900 dark:text-gray-900">{{ formatDate(work.publishedAt) }}</span>
